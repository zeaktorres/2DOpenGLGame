...
void Grid::init(float MAXDISTANCE, float ROWS, float COLUMNS, float SPACING, std::vector<int> COLORS) {
    ...
    for (float ii = MAXDISTANCE * 2 / ROWS; ii <= MAXDISTANCE * 2; ii += MAXDISTANCE * 2 / ROWS) {
        for (float i = MAXDISTANCE * 2 / COLUMNS; i <= MAXDISTANCE * 2; i += MAXDISTANCE * 2 / COLUMNS) {
            rectangleBottomRight = Point(-MAXDISTANCE + i - SPACING, -MAXDISTANCE + ii - SPACING);
            rectangleTopleft = Point(-MAXDISTANCE + SPACING + i - (MAXDISTANCE * 2 / COLUMNS), -MAXDISTANCE + SPACING + ii - (MAXDISTANCE * 2 / ROWS));
            colorOfSquare = std::vector<int>{ COLORS.at(0), COLORS.at(1), COLORS.at(2) };
            squares.push_back(Square(rectangleTopleft, rectangleBottomRight, colorOfSquare));
        }
